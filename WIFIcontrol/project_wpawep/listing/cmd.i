#line 1 "..\\marvel\\driver\\src\\cmd.c"
...
#line 2 "..\\marvel\\driver\\src\\cmd.c"
...
#line 3 "..\\marvel\\driver\\src\\cmd.c"
...
#line 331 "..\\marvel\\driver\\inc\\dev.h"
...
#line 383 "..\\marvel\\driver\\inc\\dev.h"
...
#line 400 "..\\marvel\\driver\\inc\\dev.h"
...
#line 4 "..\\marvel\\driver\\src\\cmd.c"
...
#line 5 "..\\marvel\\driver\\src\\cmd.c"
#line 6 "..\\marvel\\driver\\src\\cmd.c"
#line 7 "..\\marvel\\driver\\src\\cmd.c"
...
#line 8 "..\\marvel\\driver\\src\\cmd.c"
...
#line 9 "..\\marvel\\driver\\src\\cmd.c"
#line 10 "..\\marvel\\driver\\src\\cmd.c"
...
int lbs_allocate_cmd_buffer(struct lbs_private *priv)
{
	...
	static u8 gmarvel_cmdarray[sizeof(struct cmd_ctrl_node) * 1];
	static u8 gmarvel_cmdbuffer[1][1024];
	struct cmd_ctrl_node *cmdarray;

	;

	 
	bufsize = sizeof(struct cmd_ctrl_node) * 1;
	...
	for (i = 0; i < 1; i++) {
		...
		memset(cmdarray[i].cmdbuf,0,1024);
	}
		     ...
		     s16 *maxlevel)
{
	...
	ret = lbs_cmd_with_response(priv, 0x001e, &cmd);
	if (ret == 0) {
		*curlevel = (cmd . curlevel);
		if (minlevel)
...
void lbs_set_mac_control(struct lbs_private *priv)
{
	...
	lbs_cmd_async(priv, 0x0028, &cmd.hdr, sizeof(cmd));
...
int lbs_update_hw_spec(struct lbs_private *priv)
{
	...
	memset(&cmd, 0, sizeof(cmd));
	cmd.hdr.size = (sizeof(cmd));
	memcpy(cmd.permanentaddr, priv->current_addr, 6);
	
	 ret=lbs_cmd_with_response(priv,0x0003,(void *)&cmd);
	if (ret)
		goto out;
	...
	;
#line 336 "..\\marvel\\driver\\src\\cmd.c"
	;
	...
	if (((priv->fwrelease)>>24) == (0x05))
		priv->mesh_fw_ver = MESH_FW_OLD;
	else if ((((priv->fwrelease)>>24) >= (0x0a)) &&
		(priv->fwcapinfo & (1<<16)))
	...
	if (((priv->fwrelease)>>24) == (0x04))
		priv->regioncode = ((cmd . regioncode) >> 8) & 0xFF;
	else
		priv->regioncode = (cmd . regioncode) & 0xFF;
	...
	if (priv->current_addr[0] == 0xff)
		...
		memmove(priv->current_addr, cmd.permanentaddr, 6);
	...
	unsigned long callback_arg)
{
	...
	cmdnode->cmdbuf->size    = (in_cmd_size);
	cmdnode->cmdbuf->seqnum  = (priv->seqnum);
	cmdnode->cmdbuf->result  = 0;

	
	printf("PREP_CMD: command 0x%04x\n", command);
...
re_while:
	...
	ret = cmdnode->result;
	if (ret)
		printf("PREP_CMD: command 0x%04x failed: %d\n",
			    command, ret);
		     ...
		     struct cmd_header *resp)
{
	...
	copy_len = ((((buf->size))<((resp->size)))? ((buf->size)):((resp->size)));
	...
	case (0x8000 | 0x0019):
		{
			...
			priv->offsetvalue.offset = (u32)(reg->offset);
				...
				struct cmd_ds_command *resp)
{
	...
	priv->SNR[TYPE_BEACON][TYPE_NOAVG] = ((__le16)(*(__le16 *)&rssirsp->SNR));
	priv->NF[TYPE_BEACON][TYPE_NOAVG] = ((__le16)(*(__le16 *)&rssirsp->noisefloor));

	priv->SNR[TYPE_BEACON][TYPE_AVG] = ((__le16)(*(__le16 *)&rssirsp->avgSNR));
	priv->NF[TYPE_BEACON][TYPE_AVG] = ((__le16)(*(__le16 *)&rssirsp->avgnoisefloor));

	priv->RSSI[TYPE_BEACON][TYPE_NOAVG] =
	    ((s32)((s32)(priv->SNR[TYPE_BEACON][TYPE_NOAVG]) + ((s32)(-(s32)(priv->NF[TYPE_BEACON][TYPE_NOAVG])))));


	priv->RSSI[TYPE_BEACON][TYPE_AVG] =
	    ((s32)((s32)(priv->SNR[TYPE_BEACON][TYPE_AVG] / 100) + ((s32)(-(s32)(priv->NF[TYPE_BEACON][TYPE_AVG] / 100)))));
					...
					struct cmd_ds_command *resp)
{
	...
	if (bcn_ctrl->action == 0x0000) {
		priv->beacon_enable = (u8) (bcn_ctrl->beacon_enable);
		priv->beacon_period = (bcn_ctrl->beacon_period);
	}
				      ...
				      struct cmd_header *cmd_response)
{
	...
	case (0x8000 | 0x005b):
		...
		printf("failed CMD_802_11D_DOMAIN_INFO\n");
		while(1){
			if(0x005b!=respcmd)
				break;
		}
	...
	default:
		printf("CMD_RESP: unknown cmd response 0x%04x\n",
			   (resp->command));
		break;
	}
	 ;
...
int lbs_process_command_response(struct lbs_private *priv, u8 *data, u32 len)
{
	...
	if (respcmd != (0x8000 | curcmd) &&
	    respcmd != 0x8012 && curcmd != 0x0050) {
		;
...
int lbs_execute_next_command(struct lbs_private *priv)
{
	struct cmd_ctrl_node *cmdnode = 0;
	struct cmd_header *cmd;
	...
	if (!list_empty(&priv->cmdpendingq)) {
		cmdnode = ((struct cmd_ctrl_node *)(((char *)(&priv->cmdpendingq)->next) - ((unsigned long)(&((struct cmd_ctrl_node *)0)->list))));
	...
	if (cmdnode) {
		...
		if (is_command_allowed_in_ps((cmd->command))) {
			...
			if (cmd->command != (0x0021)) {
				...
				if ((priv->psstate == PS_STATE_SLEEP)
					...
					printf("power down mode,cann't exec cmd!\n");
					do {if(0) break;}while(1);
					}	
...
#line 1111 "..\\marvel\\driver\\src\\cmd.c"
				   ...
				   u16 cmd_action)
{
	...
	cmd->command = (0x0021);
	cmd->size = (sizeof(struct cmd_ds_802_11_ps_mode) + sizeof(struct cmd_ds_gen));

	psm->action = (cmd_action);
	psm->multipledtim = 0;
	switch (cmd_action) {
	case 0x0030:
		;
	...
	case 0x0019:
		{
			...
			cmdptr->size =
			    (sizeof (struct cmd_ds_mac_reg_access) + sizeof(struct cmd_ds_gen));

			macreg =
			    (struct cmd_ds_mac_reg_access *)&cmdptr->params.
			    macreg;

			macreg->action = (cmd_action);
			macreg->offset = ((u16) offval->offset);
			macreg->value = (offval->value);
	...
	case 0x001a:
		{
			...
			cmdptr->size =
			    (sizeof (struct cmd_ds_bbp_reg_access) + sizeof(struct cmd_ds_gen));
			...
			bbpreg->action = (cmd_action);
			bbpreg->offset = ((u16) offval->offset);
	...
	case 0x001b:
		{
			...
			cmdptr->size =
			    (sizeof (struct cmd_ds_rf_reg_access) + sizeof(struct cmd_ds_gen));
			...
			rfreg->action = (cmd_action);
			rfreg->offset = ((u16) offval->offset);
				      ...
				      u16 cmd_action, void *pdata_buf)
{
	...
	cmd->command = (0x0098);
	cmd->size =
	    (sizeof(struct cmd_ds_802_11_monitor_mode) + sizeof(struct cmd_ds_gen));


	monitor->action = (cmd_action);
				...
				struct cmd_ds_command *cmd)
{
	...
	;
	cmd->command = (0x001f);
	cmd->size = (sizeof(struct cmd_ds_802_11_rssi) + sizeof(struct cmd_ds_gen));
	cmd->params.rssi.N = (8);
			       ...
			       u16 cmd_action, void *pdata_buf)
{
	...
	cmd->command = (0x0087);
	cmd->size = ((sizeof(struct cmd_ds_bt_access)+sizeof(struct cmd_ds_gen)));
	cmd->result = 0;
	bt_access->action = (cmd_action);

	switch (cmd_action) {
	case CMD_ACT_BT_ACCESS_ADD:
		memcpy(bt_access->addr1, pdata_buf, 2 * 6);
		lbs_hex(0x00000010, "BT_ADD: blinded MAC addr", bt_access->addr1, 6);
		break;
	case CMD_ACT_BT_ACCESS_DEL:
		memcpy(bt_access->addr1, pdata_buf, 1 * 6);
		lbs_hex(0x00000010, "BT_DEL: blinded MAC addr", bt_access->addr1, 6);
		break;
	case CMD_ACT_BT_ACCESS_LIST:
		bt_access->id = (*(u32 *) pdata_buf);
		break;
			       ...
			       u16 cmd_action, void *pdata_buf)
{
	...
	cmd->command = (0x0095);
	cmd->size = ((sizeof(struct cmd_ds_fwt_access)+sizeof(struct cmd_ds_gen)));
	cmd->result = 0;
				...
				u16 cmd_action)
{
	...
	;
	cmd->size =
	    (sizeof(struct cmd_ds_802_11_beacon_control) + sizeof(struct cmd_ds_gen));

	cmd->command = (0x00b0);

	bcn_ctrl->action = (cmd_action);
	bcn_ctrl->beacon_enable = (priv->beacon_enable);
	bcn_ctrl->beacon_period = (priv->beacon_period);
			  ...
			  u16 wait_option, u32 cmd_oid, void *pdata_buf)
{
	...
	case 0x003d:
		...
		cmdptr->command = (cmd_no);
		cmdptr->size = (sizeof(struct cmd_ds_802_11_afc) + sizeof(struct cmd_ds_gen));
		...
		ret = 0;
		goto done;
#line 1485 "..\\marvel\\driver\\src\\cmd.c"
	case 0x0072:
		cmdptr->command = (0x0072);
		cmdptr->size =
		    (sizeof(struct cmd_ds_802_11_tpc_cfg) + sizeof(struct cmd_ds_gen));
	...
	case 0x004e:
		{
			...
			cmdptr->size =
			    ((gpio->header . len) + sizeof(struct cmd_ds_gen) + 8);
	...
	case 0x0080:
		cmdptr->command = (0x0080);
		cmdptr->size = (sizeof(struct cmd_ds_get_tsf) + sizeof(struct cmd_ds_gen));
	...
	case 0x00b0:
		...
		break;
	default:
		printf("PREP_CMD: unknown command 0x%04x\n", cmd_no);
		ret = -1;
		break;
	}
...
int lbs_get_channel(struct lbs_private *priv)
{
	...
	ret = lbs_cmd_with_response(priv, 0x001d, &cmd);
	if (ret)
		goto out;
...
int lbs_set_channel(struct lbs_private *priv, u8 channel)
{
	...
	priv->curbssparams.channel = (uint8_t) (cmd . channel);
	;
			   ...
			   struct assoc_request *assoc)
{
	...
	memset(&cmd, 0, sizeof(cmd));
	cmd.hdr.command = (0x0013);
	cmd.hdr.size = (sizeof(cmd));

	cmd.action = (cmd_action);

	if (cmd_action == 0x0002) {
		int i;

		 
		cmd.keyindex = (assoc->wep_tx_keyidx & 0x3fff);
	...
	} else if (cmd_action == 0x0004) {
		 

		 
		cmd.keyindex = (priv->wep_tx_keyidx & 0x3fff);

		;
	}

	ret = lbs_cmd_with_response(priv, 0x0013, &cmd);
done:
	;
	return ret;
}
			      ...
			      uint16_t *enable)
{
	...
	ret = lbs_cmd_with_response(priv, 0x002f, &cmd);
	if (!ret && cmd_action == 0x0000)
		*enable = (cmd . enable);
                            ...
                            struct enc_key *key)
{
	...
	if (key->flags & KEY_INFO_WPA_ENABLED)
		keyparam->keyinfo |= (KEY_INFO_WPA_ENABLED);
	if (key->flags & KEY_INFO_WPA_UNICAST)
		keyparam->keyinfo |= (KEY_INFO_WPA_UNICAST);
	if (key->flags & KEY_INFO_WPA_MCAST)
		keyparam->keyinfo |= (KEY_INFO_WPA_MCAST);

	keyparam->type = ((0x0100 + 0));
	keyparam->keytypeid = (key->type);
	keyparam->keylen = (key->len);
	memcpy(keyparam->key, key->key, key->len);

	 
	keyparam->length = (sizeof(*keyparam) - 4);
	;
}
				...
				struct assoc_request *assoc)
{
	...
	if (cmd_action == 0x0000) {
		cmd.hdr.size = (sizeof(struct cmd_ds_gen) + 2);
	} else {
		memset(cmd.keyParamSet, 0, sizeof(cmd.keyParamSet));

		if (test_bit(9, &assoc->flags)) {
			set_one_wpa_key(&cmd.keyParamSet[index],
					&assoc->wpa_unicast_key);
			index++;
		}

		if (test_bit(8, &assoc->flags)) {
			set_one_wpa_key(&cmd.keyParamSet[index],
					&assoc->wpa_mcast_key);
			index++;
		}
		...
		cmd.hdr.size = (((size_t) &((struct cmd_ds_802_11_key_material *)0)->keyParamSet[index]));

	}
	ret = lbs_cmd_with_response(priv, 0x005e, &cmd);
	...
	if (!ret && cmd_action == 0x0000) {
		...
		while (buf_ptr < resp_end) {
			...
			struct enc_key *key;
			uint16_t param_set_len = (keyparam->length);
			uint16_t key_len = (keyparam->keylen);
			uint16_t key_flags = (keyparam->keyinfo);
			uint16_t key_type = (keyparam->keytypeid);
			char *end;
...
int lbs_set_radio(struct lbs_private *priv, u8 preamble, u8 radio_on)
{
	...
	ret = lbs_cmd_with_response(priv, 0x001c, &cmd);
...
#line 2023 "..\\marvel\\driver\\src\\cmd.c"
...
int lbs_set_snmp_mib(struct lbs_private *priv, u32 oid, u16 val)
{
	...
	switch (oid) {
	case 0x0000:
		cmd.bufsize = (sizeof(u8));
		cmd.value[0] = (val == 1) ? 2 : 1;
		break;
	case 0x0009:
	case 0x0008:
	case 0x0005:
	...
	ret = lbs_cmd_with_response(priv, 0x0016, &cmd);
...
int lbs_remove_wep_keys(struct lbs_private *priv)
{
    struct cmd_ds_802_11_set_wep cmd;
    int ret;
...
int clear_wpa_wpa2_info(struct lbs_private *priv)
{
        if (lbs_set_key_material(priv,KEY_TYPE_ID_WEP,2,0,0) != 0)
                return -1;
        if (lbs_set_key_material(priv,KEY_TYPE_ID_WEP,1,0,0) != 0)
                return -1;
        return 0;
}

int lbs_set_key_material(struct lbs_private *priv,
				int key_type,
				int key_info,
				u8 *key, u16 key_len)
{
	...
	cmd.hdr.size = (sizeof(cmd));
	cmd.action = (0x0001);
	cmd.param.type = ((0x0100 + 0));
	cmd.param.length = (sizeof(cmd . param) - 4);
	cmd.param.keytypeid = (key_type);
	cmd.param.keyinfo = (key_info);
